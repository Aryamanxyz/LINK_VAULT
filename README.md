...
app.ts – global middlewares (express.json(), cookie-parser()).
router – attaches endpoint‑specific middlewares (authMiddleware, rateLimiter).
controller – validates payload with Zod, calls the corresponding service.
service – executes Prisma queries, optionally writes/reads from Redis cache.
service – returns the result to the controller.
controller – wraps the data in a standard ApiResponse and sends JSON back.
...
login issues both tokens; the refresh token is saved in the user's record.
authMiddleware verifies the access token; on expiry it reads the refresh token, validates it, checks it matches the DB entry, issues a new access token, and continues.
Logout clears the refresh token from the DB and removes both cookies.
...
Workflow:
On GET /api/links/:code, first attempt redis.get(key).
If a cache hit, redirect instantly.
On miss, query Prisma, then redis.set(key, value, { EX: TTL }).
...
Checkout code.
Set up Node.js (v20) with npm cache.
